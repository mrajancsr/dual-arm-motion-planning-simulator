#!/usr/bin/env python3
"""
Interactive RRT* Path Planning with Obstacles for 2-Link Arm
...
def solve_left_arm_ik(arm, target_ee, base, max_attempts=30):
    ...
    guesses = [
        ...
        (theta_to_target + 0.1, 0.0),
        (theta_to_target - 0.1, 0.0),
        (theta_to_target + 0.2, 0.0),
        (theta_to_target - 0.2, 0.0),
    ]
    
    # Try different tolerances (relaxed for 2-link arm)
...
def interactive_rrt_planner():
    ...
    print("=" * 70)
    print("Interactive RRT* Path Planning - 2-Link Left Arm")
    print("=" * 70)
    print("\nInstructions:")
    ...
    left_arm = TwoLinkArm(L1=0.7, L2=0.6, name="Left2Link")
    right_arm = TwoLinkArm(L1=0.7, L2=0.6, name="Right2Link")
    ...
    def run_planner(event):
        ...
        try:
            ...
            # For 2-link arms, smaller step_size and goal_threshold work well
            planner = RRTStar(
                planner_state['dual_arm'],
                max_iterations=15000,  # Fewer iterations needed for 2-link
                step_size=0.2,  # Smaller step size for 2-link arms
                goal_threshold=0.3,  # Smaller goal threshold for 2-link arms
                rewire_radius=0.4,  # Smaller rewire radius
                verbose=True
            )
            ...
            # Update status
            planner_state['status_text'].set_text(f'Path found! Starting animation...')
